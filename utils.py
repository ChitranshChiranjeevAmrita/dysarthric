...
def feature_extraction(filepath,sr=16000, min_dur_sec=1 ,win_length=400,hop_length=80, n_mfcc=40, n_mels=40, spec_len=400,mode='dev'):
    audio_data = load_wav(filepath, sr=sr, input_length_time=1)
    linear_spect = lin_mfcc_from_wav(audio_data, hop_length, win_length, n_mfcc, n_fft=512)
    #linear_spect = lin_mel_from_wav(audio_data, hop_length, win_length, n_mels)
...
def path_contains_folder(path, folder_name):
    # Split the path into its components
    components = os.path.normpath(path).split(os.path.sep)

    # Check if the folder_name is in the components
    return folder_name in components
    
    
    
    
def load_data(filepath,sr=16000, min_dur_sec=4,win_length=400,hop_length=80, n_fft=512, n_mels=40, spec_len=400,n_mfcc=512, mode='train'):
